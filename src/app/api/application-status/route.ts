...
// Look up a center application in the admin app's Supabase database by its
// application number + email, and return a sanitized status (plus a payment
// link when the fee is still unpaid). Reads use the public anon key, so the
// query is restricted to the small set of fields a center needs to see.
...
const SELECT_FIELDS =
  "application_no,center_name,email,approval_status,payment_status," +
